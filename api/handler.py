...
def _send_email_report(to_email: str, name: str, analysis: str, profile: dict) -> dict:
    ...
    if profile:
        ...
        <table style="width:100%;border-collapse:collapse;margin:16px 0;font-family:Arial,sans-serif;">
            <tr><td style="padding:8px;color:#666;">Diabetes Type:</td><td style="padding:8px;font-weight:bold;">{profile.get('diabetesType') or profile.get('dtype', 'N/A')}</td></tr>
            <tr><td style="padding:8px;color:#666;">Age:</td><td style="padding:8px;font-weight:bold;">{profile.get('age', 'N/A')}</td></tr>
            <tr><td style="padding:8px;color:#666;">HbA1c:</td><td style="padding:8px;font-weight:bold;">{profile.get('hba1c', 'N/A')}%</td></tr>
            <tr><td style="padding:8px;color:#666;">Weight:</td><td style="padding:8px;font-weight:bold;">{profile.get('weight', 'N/A')} kg</td></tr>
            <tr><td style="padding:8px;color:#666;">Medications:</td><td style="padding:8px;font-weight:bold;">{profile.get('meds') or profile.get('medications', 'N/A')}</td></tr>
            <tr><td style="padding:8px;color:#666;">Goal HbA1c:</td><td style="padding:8px;font-weight:bold;">{profile.get('goalHba1c', 'N/A')}%</td></tr>
        </table>
        """

    # Sanitize analysis for HTML
    analysis_html = analysis.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\n", "<br>")

    # Build a deliverability-friendly HTML email
...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:Arial,Helvetica,sans-serif;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:20px;">
        <tr><td align="center">
            <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;">
                <tr><td style="background:#16a34a;padding:24px;text-align:left;">
                    <h1 style="margin:0;color:#ffffff;font-size:22px;">Diabetes Care Health Report</h1>
                    <p style="margin:6px 0 0;color:#dcfce7;font-size:13px;">Personalized health summary</p>
                </td></tr>
                <tr><td style="padding:24px;color:#374151;font-size:14px;line-height:1.6;">
                    <p>Hello {name},</p>
                    <p>Below is your personalized health analysis from the Diabetes Care application.</p>
                    
                    <h3 style="color:#111827;font-size:15px;margin:20px 0 8px;">Profile Summary</h3>
                    {profile_summary}
                    
                    <h3 style="color:#111827;font-size:15px;margin:20px 0 8px;">Analysis Details</h3>
                    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:16px;color:#374151;font-size:13px;line-height:1.7;">
                        {analysis_html}
                    </div>
                    
                    <p style="margin-top:24px;color:#6b7280;font-size:12px;">
                        <strong>Important:</strong> This report is generated for informational purposes only. Please consult with your healthcare provider for medical advice.
                    </p>
                </td></tr>
                <tr><td style="background:#f9fafb;padding:16px;text-align:center;color:#9ca3af;font-size:11px;border-top:1px solid #e5e7eb;">
                    Diabetes Care Application | Powered by AWS
                </td></tr>
            </table>
        </td></tr>
    </table>
    ...
    try:
        ses.send_email(
            Source="Diabetes Care <ESSALEM.SIDI.MOHAMED@student.mmu.edu.my>",
            Destination={"ToAddresses": [to_email]},
            ReplyToAddresses=["ESSALEM.SIDI.MOHAMED@student.mmu.edu.my"],
            Message={
                "Subject": {"Data": f"Health Report for {name}", "Charset": "UTF-8"},
                "Body": {
                    "Html": {"Data": html_body, "Charset": "UTF-8"},
                    "Text": {"Data": f"Hello {name},\n\nBelow is your health analysis from Diabetes Care:\n\n{analysis}\n\n---\nThis is an automated health report. Please consult your healthcare provider for medical advice.", "Charset": "UTF-8"},
                }
            }
        )
